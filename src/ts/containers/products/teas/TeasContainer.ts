...
export default class TeasContainer extends BaseContainer {
    ...
    async onInit(){
        ...
        teas.forEach((tea: any) => {
            ...
            teasListSection.insertAdjacentHTML("beforeend", cardComponent(tea,  teaCardButtons),)
            const addToCartButton = document.getElementById(`add-to-cart-tea-card-button-${tea.id}`)
            addToCartButton.onclick = () => addToCartHandler({ id: tea.id, name: tea.name, price: tea.price }, this)
        })
