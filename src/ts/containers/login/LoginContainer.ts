...
class LoginContainer extends BaseContainer {
  ...
  onInit(instance: LoginContainer) {
    ...
    if (this.isLoggingIn) {
      ...
      formFileSection.insertAdjacentHTML(
        "afterbegin",
        formFileComponent({
          id: "photo",
  ...
  async onSubmit(event: SubmitEvent) {
    ...
    if (this.isLoggingIn) {
      ...
      const hasLoggedIn = await authService.login({
        email: emailValue,
      ...
      formResult.innerText =
        "You have successfully logged in... Redirection to the login page in 2s";
      setTimeout(() => {
      ...
      formResult.innerText =
        "You have successfully signed up... Redirection to the login page in 2s";
      setTimeout(() => {
